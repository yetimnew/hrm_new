                ...
                <div class="card text-left col-md-12">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            <h4>All leave Periods</h4>
                            {{-- @can('customer create') --}}

                            <div class="ml-auto">
                                <a href="{{route('leave_period.create')}}" class="btn btn-outline-primary btn-sm"><i
                                        class="fas fa-plus mr-1"></i>Add Leave Period</a>
                    ...
                    <div class="card-body">
                        <div class="table-responsive text-nowrap">
                            <table class="table-sm table table-bordered table-sm table-striped" id="personales">
                                ...
                                <tbody>
                                    <?php $no = 0 ?>
                                    @if ($leave_periods->count() > 0)
                                    @foreach ($leave_periods as $leave_period)
                                    <tr>
                                        <input type="hidden" class="deleted_value_id" value="{{$leave_period->id}}">
                                        <td class='p-1'>{{++$no}}</td>
                                        <td class='p-1'>{{$leave_period->name}}</td>
                                        <td class='p-1'>{{$leave_period->start_date}}</td>
                                        <td class='p-1'>{{$leave_period->end_date}}</td>


                                        {{-- @can('driver edit') --}}
                                        <td class='p-1 text-center' data-toggle="tooltip" data-placement="top"
                                            title="edit">
                                            <a href="{{route('leave_period.edit', $leave_period->id)}}"><i
    ...
    var deleted_id = $(this).closest("tr").find('.deleted_value_id').val();
        ...
        swal({
            ...
            if (willDelete) {
                ...
                var urlPath = '/hrm/leave_period/'+ deleted_id;
                // alert(urlPath);
