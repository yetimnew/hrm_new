                ...
                <div class="card text-left col-md-12">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="ml-auto">
                                <a href="{{route('branch.create')}}" class="btn btn-outline-primary btn-sm"><i
                                        class="fas fa-plus mr-1"></i>Add branch</a>
                        ...
                        <div class="table-responsive text-nowrap">
                            <table class="table-sm table table-bordered table-sm table-striped" id="personales">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <input type="hidden" class="deleted_value_id" value="{{$branch->id}}">
                                        <td class='p-1'>{{++$no}}</td>
                                        <td class='p-1'>{{$branch->name}}</td>
                                        <td class='p-1'>{{$branch->city}}</td>
                                        <td class='p-1'>{{$branch->address}}</td>
                                        <td class='p-1'>{{$branch->phone}}</td>
                                        <td class='p-1'>{{$branch->fax}}</td>

                                        {{-- @can('driver edit') --}}
                                        <td class='p-1 text-center' data-toggle="tooltip" data-placement="top"
                                            title="edit">
                                            <a href="{{route('branch.edit', $branch->id)}}"><i
    ...
    var deleted_id = $(this).closest("tr").find('.deleted_value_id').val();
        ...
        swal({
            ...
            if (willDelete) {
                ...
                var urlPath = '/hrm/branch/'+ deleted_id;
                // alert(urlPath);
